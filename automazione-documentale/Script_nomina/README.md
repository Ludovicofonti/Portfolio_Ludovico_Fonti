...
Generatore Nomina/
├── config/
│   ├── Configurazione sezioni variabili + prompt RAG
│   └── Registry anagrafica enti
├── core/
│   ├── Orchestratore pipeline
│   ├── Engine estrazione (Fase 1)
│   ├── Processamento dati e smart retry
│   ├── Compilatore (Fase 2 alternativa)
│   ├── Template processor DOCX
│   ├── Registry per risoluzione dati
│   ├── Modelli di dominio
│   └── Gerarchia eccezioni
├── utilities/
│   ├── Client embedding
...
├── data/
│   ├── Output OCR raw
│   └── Dati strutturati post-elaborazione
├── Documenti sorgente (input)
└── output/
    ├── Dati intermedi
    └── Documenti finali
```
